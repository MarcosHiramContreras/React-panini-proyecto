...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    // Obtener el ID del distribuidor a eliminar
    $id = isset($_GET['id']) ? $_GET['id'] : null;

    if ($id === null) {
        // Si no se proporciona el ID, responder con un mensaje de error
        http_response_code(400); // Bad Request
        echo json_encode(array('message' => 'No se proporcionó el ID del distribuidor.'));
        exit();
    }
    ...
    // Preparar la consulta para eliminar el distribuidor
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo json_encode(array('message' => 'Distribuidor eliminado correctamente'));
        } else {
            // Si no se encontró el distribuidor con el ID proporcionado, responder con un mensaje de error
            http_response_code(404); // Not Found
            echo json_encode(array('message' => 'No se encontró el distribuidor con el ID proporcionado'));
        }
    } catch (PDOException $e) {
        // Si hay un error en la ejecución de la consulta, responder con un mensaje de error
        http_response_code(500); // Internal Server Error
        echo json_encode(array('message' => 'Error al eliminar el distribuidor: ' . $e->getMessage()));
    }
} else {
